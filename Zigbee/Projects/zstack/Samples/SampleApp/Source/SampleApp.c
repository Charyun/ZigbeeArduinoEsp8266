...
void SampleApp_SendPeriodicMessage( void )
{
  ...
  unsigned char temp[3], humidity[3], strTemp[9];
...
//------------------------------------------------
  ...
  humidity[2] = '\0';
  //将数据整合后方便发给协调器显示
  osal_memcpy(strTemp, "T", 1);
  osal_memcpy(&strTemp[1], temp, 2); 
  osal_memcpy(&strTemp[3], " H", 2);
  osal_memcpy(&strTemp[5], humidity, 3);
 ...
 */
 /*
  osal_memcpy(data, strTemp, 5); 
  osal_memcpy(&data[5], " ", 1); 
  osal_memcpy(&data[6], pmval_s, pmval_s_len); 
  */
  osal_memcpy(data, strTemp, 7); 
  osal_memcpy(&data[7], " P", 2); 
  osal_memcpy(&data[9], pmval_s, pmval_s_len); 
  data[10+pmval_s_len] = 'E';
